...
import { BN, getAddress, WoopAddress } from '@woop-js/crypto';
import { hexToNumber } from '@woop-js/utils';
import { Messenger, RPCMethod, Emitter, HttpProvider, NewHeaders } from '@woop-js/network';
...
export class TransactionBase implements AbstractTransaction {
  static normalizeAddress(address: string) {
    if (address === '0x') {
      ...
      WoopAddress.isValidChecksum(address) ||
      WoopAddress.isValidBech32(address) ||
      WoopAddress.isValidBech32TestNet(address)
    ) {
    ...
    return new Promise((resolve, reject) => {
      ...
      newHeads.then((p) => {
        p.onData(async (data: any) => {
          const blockNumber =
            this.messenger.chainPrefix === 'wiki'
