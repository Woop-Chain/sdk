...
import { add0xToString, numberToHex, ChainType, Unit } from '@woop-js/utils';
...
class Transaction extends TransactionBase {
  ...
  getRLPUnsigned(): [string, any[]] {
    ...
    const fields =
      this.messenger.chainType === ChainType.Woop ? transactionFields : transactionFieldsETH;
  ...
  getRLPSigned(raw: any[], signature: Signature): string {
    // temp setting to be compatible with eth
    const rawLength = this.messenger.chainType === ChainType.Woop ? 11 : 9;
  ...
  recover(rawTransaction: string): Transaction {
    // temp setting to be compatible with eth
    const recovered =
      this.messenger.chainType === ChainType.Woop
   ...
   * ```
   * txn.setParams({
   *   to: 'woop1ew56rqrucu6p6n598fmjmnfh8dd4xpg6atne9c',
   *   value: '1200',
   *   gasLimit: '230000',
   *   shardID: 1,
   *   toShardID: 0,
   *   gasPrice: new wiki.utils.Unit('101').asGwei().toWei(),
   *   signature: {
